...
def add_subparser(subparsers: _SubParsersAction[ArgumentParser]):
    ...
    subparser.add_argument("--temperature", "-t", type=float, default=0, help="Standard deviation of the noise to add to the centroid [0,1]")
...
def command(args):
    ...
    query = Query(proximity_function=args.proximity, limit=args.limit, path=args.path, json=args.json, songId=args.songId, albumId=args.albumId, artistId=args.artistId, temperature=args.temperature)
